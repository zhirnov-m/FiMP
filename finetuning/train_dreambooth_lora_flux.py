# based on https://github.com/huggingface/diffusers/blob/main/examples/dreambooth/train_dreambooth_lora_flux.py
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "path to csv file with enhanced prompts for KREAM dataset"
        ),
    )
    parser.add_argument(
        "--dataset_config_name",
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if args.dataset_name is not None:
            ...
            if args.enhanced_prompts is not None:
                dataframe = pd.read_csv(args.enhanced_prompts)
                caption_mapping = dict(zip(dataframe["original_caption"], dataframe["enhanced_caption"]))

                def update_caption(example):
                    image_id = example[args.caption_column]
                    assert len(example[args.caption_column].split(', a photography of')) == 2
                    example[args.caption_column] = example[args.caption_column].split(', a photography of')[0] + \
                                                ". " + caption_mapping[image_id]

                    return example

                dataset["train"] = dataset["train"].map(update_caption)
                print("Improved text prompts have been uploaded!")
        ...
        # for image in tqdm(self.instance_images, desc="Making image transforms",
        #                   disable=verbose):
...
def main(args):
    ...
    train_dataset = DreamBoothDataset(
        ...
        verbose=not accelerator.is_local_main_process,
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                # create pipeline
                weight_dtype_validation = torch.float16
                if not args.train_text_encoder:
                    text_encoder_one, text_encoder_two = load_text_encoders(text_encoder_cls_one, text_encoder_cls_two)
                    text_encoder_one.to(weight_dtype_validation)
                    text_encoder_two.to(weight_dtype_validation)
                pipeline = FluxPipeline.from_pretrained(
                    args.pretrained_model_name_or_path,
                    vae=vae,
                    ...
                    revision=args.revision,
                    variant=args.variant,
                    torch_dtype=weight_dtype_validation,
                )
